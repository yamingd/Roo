...
class Memcache(CacheBase):
    ...
    def __init__(
        self, servers, debug=False, noreply=False, no_block=True,
        ...
        logger.info('init memcache client: %s' % servers)
    ...
    def add(self, key, value, time=0, herd=True):
        try:
            with self.clients.reserve() as mc:
                ...
                # timestamp information into the object to be persisted into
                # memcached
                if herd and time != 0:
            ...
            logger.exception('unexpected error:%s' % key)
            return None
    ...
    def delete(self, key):
        try:
            with self.clients.reserve() as mc:
                logger.debug("delete cache %s" % key)
    ...
    def incr(self, key, delta=1):
        try:
            with self.clients.reserve() as mc:
                ...
                logger.debug("incr %s (delta=%s) ret=%s" % (key, delta, ret))
                return ret
        except:
            logger.exception('unexpected error:%s' % key)
            return None
    ...
    def decr(self, key, delta=1):
        try:
            with self.clients.reserve() as mc:
                ...
                logger.debug("decr %s (delta=%s) %s" % (key, delta, ret))
                return ret
        except:
            logger.exception('unexpected error:%s' % key)
            return None
        ...
        try:
            with self.clients.reserve() as mc:
                ...
                for key, packed in packed_resp.iteritems():
                    # If it was a miss, treat it as a miss to our response &
                    # continue
                    if packed is None:
                        resp[key] = packed
                        continue
