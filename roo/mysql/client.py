...
class Connection(object):
    ...
    def __repr__(self):
        return "MySQL (%s/%s)" % (self.host, self.database)
    ...
    def show(self, table_name):
        ...
        cursor = self._cursor()
        try:
            self._execute(cursor, query, None)
            column_names = [d[0] for d in cursor.description]
            return [Row(itertools.izip(column_names, row)) for row in cursor]
        except:
            logger.exception('unexpected error, show table=' + table_name)
            raise
        finally:
            cursor.close()
    ...
    def executemany(self, query, parameters):
        ...
        cursor = self._cursor()
        try:
            dbname = threadlocal.get('dbname')
            if dbname:
                cursor.execute('USE %s' % dbname)
                if self.debug:
                    logger.debug('USE %s' % dbname)
            else:
                if self.debug:
                    logger.debug('dbname is None.')
...
class ConnectionProxy(object):
    ...
    def __init__(self, host='127.0.0.1:3306', database=None, num_clients=5, user=None, passwd=None, debug=False):
        self.host = host
        self.database = database
        self.clients = ConnectionPool(n_slots=num_clients)
        self.user = user
        self.passwd = passwd
        self.debug = debug
        for x in xrange(num_clients):
            self.clients.put(self.build())
        logger.info('init MySQL Connection Proxy')

    def build(self):
        r = Connection(self.host, self.database, user=self.user,
                       password=self.passwd, debug=self.debug)
        return r
...
def instrument(name):
    def do(self, *args, **kwargs):
        try:
            ...
            logger.exception('unexpected error:%s', args)
            raise
    return do
