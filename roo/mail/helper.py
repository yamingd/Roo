...
class EmailMessage(object):
    ...
    def __init__(self, subject, body, from_email, to=None, bcc=None,
        ...
        if to:
            assert not isinstance(
                to, basestring), '"to" argument must be a list or tuple'
            self.to = list(to)
        else:
            self.to = []
        if bcc:
            assert not isinstance(
                bcc, basestring), '"bcc" argument must be a list or tuple'
            self.bcc = list(bcc)
        else:
            self.bcc = []
    ...
    def _create_mime_attachment(self, content, mimetype):
        ...
        if basetype == 'text':
            encoding = self.encoding or settings.DEFAULT_CHARSET
            attachment = SafeMIMEText(smart_str(
                content, encoding), subtype, encoding)
        else:
...
def send_email(subject, message, from_email, recipient_list,
    ...
    connection = settings.mail.engine
