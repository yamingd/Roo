...
class AMQPJobEngine(JobEngine):
    ...
    @property
    def conf(self):
        return self.application.settings.amqp

    def connect(self):
        if self.publisher is None:
            app_id = self.application.settings.site.name
            host = self.conf.host
...
class DefaultAMQPPublisher(object):

    def __init__(self, app_id, host=None, port=5672, vhost='/', user=None, password=None, exchange=None, delivery_mode=2):
        self.host = host
        self.port = port
        self.properties = pika.BasicProperties(
            app_id=app_id, content_type="text/plain", content_encoding="utf8", delivery_mode=delivery_mode)  # persistent

        credentials = pika.PlainCredentials(user, password)
        self.parameters = pika.ConnectionParameters(
            host=host, port=port, virtual_host=vhost, connection_attempts=3, heartbeat_interval=3600, credentials=credentials)
    ...
    def publish(self, queue_name, msg):
        self._channel.basic_publish(
            exchange=self._exchange, routing_key=self._exchange + '.' + queue_name, body=msg, properties=self.properties)
...
class AsyncAMQPConsumer(object):
    ...
    def __init__(self, host='127.0.0.1',
                 ...
                 exchange_name=None, queue_name=None,
        ...
        self.ROUTING_KEY = exchange_name + '.' + queue_name
    ...
    def on_connection_closed(self, connection, reply_code, reply_text):
        ...
        if self._closing:
            ...
            self._connection.add_timeout(
                self.reconnect_timeout, self.reconnect)
    ...
    def try_reconnect(self):
        ...
        try:
            ...
            self._connection.add_timeout(
                self.reconnect_timeout, self.try_reconnect)
